...
useEffect(() => {
  const fetchJobTemplates = async () => {
    try {
      console.log("Fetching from:", "http://localhost:5000/api/jobtemplate");
  
      const response = await axios.get("http://localhost:5000/api/jobtemplate");
  
      console.log("Fetched job templates:", response.data);
      setJobTemplates(response.data);
    } catch (error) {
      console.error("Error fetching job templates:", error.response || error);
    }
  };
  fetchJobTemplates();
}, [setJobTemplates]); 
    ...
    switch (currentPage) {
      ...
      case "new-template":
        return (
          <NewJobTemplate
          setJobTemplates={setJobTemplates}
          setCurrentPage={setCurrentPage}  // Fix: Ensure setCurrentPage is passed properly
        />
        );
      case "create-folder":
